    ...
    <div className="h-screen bg-slate-950 text-slate-50 p-4 lg:p-6 overflow-auto">
      <div className="max-w-7xl mx-auto flex flex-col h-full">
        ...
        <header className="mb-4 flex-shrink-0">
          ...
          <p className="text-slate-400 text-sm">고정밀 SI 기반 공학용 단위 변환기</p>
        </header>
        ...
        <main className="grid lg:grid-cols-[280px_1fr] gap-4 flex-shrink-0">
          ...
          <section className="bg-slate-900 p-4 rounded-xl border border-slate-800">
            <h2 className="text-xs font-bold text-slate-400 mb-2 uppercase tracking-widest">Category</h2>
            <div className="grid grid-cols-3 lg:grid-cols-2 gap-1.5">
              {categories.map((cat) => (
                <button key={cat.id} onClick={() => setCategory(cat.id as CategoryId)} className={`px-2.5 py-2 rounded-lg border text-left transition ${category === cat.id ? 'border-emerald-500 bg-emerald-500/10 text-emerald-50' : 'border-slate-800 bg-slate-950 text-slate-300 hover:border-slate-700'}`}>
          ...
          <section className="bg-slate-900 p-4 lg:p-6 rounded-xl border border-slate-800">
            <div className="grid md:grid-cols-[1fr_auto_1fr] items-start gap-4">
              <div className="space-y-2">
                <label className="text-xs font-bold text-slate-400">INPUT</label>
                <input type="text" value={inputValue} onChange={(e) => setInputValue(e.target.value)} className="w-full bg-slate-950 border border-slate-800 p-3 rounded-lg text-xl outline-none focus:border-emerald-500 transition" />
                ...
                {UNIT_DESCRIPTIONS[fromUnit] && (
                  <p className="text-xs text-slate-400 leading-snug">{UNIT_DESCRIPTIONS[fromUnit]}</p>
                )}
              </div>

              <ArrowLeftRight size={22} className="text-slate-500 mx-auto mt-8" />

              <div className="space-y-2">
                <label className="text-xs font-bold text-slate-400">OUTPUT</label>
                <div className="w-full bg-slate-950 border border-slate-800 p-3 rounded-lg text-xl text-emerald-400 font-bold min-h-[3rem] flex items-center">
                ...
                {UNIT_DESCRIPTIONS[toUnit] && (
                  <p className="text-xs text-slate-400 leading-snug">{UNIT_DESCRIPTIONS[toUnit]}</p>
                )}
              </div>
            </div>
          </section>
        ...
        <div className="grid lg:grid-cols-[1fr_1fr] gap-4 mt-4 flex-1 min-h-0">
          ...
          <section className="bg-slate-900 p-4 rounded-xl border border-slate-800">
            <div className="flex items-center gap-2 mb-3">
              ...
              <h2 className="text-xs font-bold text-slate-400 uppercase tracking-widest">Quick Reference</h2>
              <span className="text-xs text-slate-500 ml-1">— {categories.find(c => c.id === category)?.label}</span>
            </div>
            <div className="space-y-1.5">
              {referenceItems.map((item, i) => (
                <div key={i} className="bg-slate-950 border border-slate-800 rounded-lg px-3 py-2.5">
          ...
          <section className="bg-slate-900 p-4 rounded-xl border border-slate-800">
            <div className="flex items-center gap-2 mb-3">
              ...
              <h2 className="text-xs font-bold text-slate-400 uppercase tracking-widest">자주 쓰는 변환</h2>
            </div>
            <div className="grid grid-cols-2 gap-1.5">
              {QUICK_CONVERSIONS.map((q, i) => (
                <button
                  ...
                  className="bg-slate-950 border border-slate-800 rounded-lg px-3 py-2.5 text-left transition hover:border-emerald-500 hover:bg-emerald-500/5 group"
                >
                  <div className="text-sm font-bold text-slate-300 group-hover:text-emerald-400 transition">{q.label}</div>
                  <div className="text-xs text-slate-500">{categories.find(c => c.id === q.category)?.label}</div>
                </button>
              ))}
            </div>
          </section>
        ...
        {/* Footer */}
        <footer className="mt-3 pb-1 text-center text-xs text-slate-500 flex-shrink-0">
